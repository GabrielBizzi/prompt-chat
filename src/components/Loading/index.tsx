...
export const LoadingComponent: React.FC<IPropsLoading> = ({
  open,
}: IPropsLoading) => {
  return (
    <>
      {open ? (
        <div className="fixed flex flex-col justify-self-center self-center h-full w-full z-50 bg-zinc-950/50">
          ...
          >
            <svg
              aria-hidden="true"
              className="w-12 h-12 text-gray-200 animate-spin dark:text-gray-600 fill-zinc-200"
              viewBox="0 0 100 101"
              fill="none"
            ...
            <span className="text-zinc-300 mt-4">Processando...</span>
          </div>
        </div>
      ) : null}
    </>
